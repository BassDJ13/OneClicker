namespace MainSettings;

internal static class SettingKeys
{
    internal const string WindowStyle     = nameof(WindowStyle);
    internal const string DockPosition    = nameof(DockPosition);
    internal const string DockOffsetX     = nameof(DockOffsetX);
    internal const string DockOffsetY     = nameof(DockOffsetY);
    internal const string InactiveOpacity = nameof(InactiveOpacity);
    internal const string FocusShortcut   = nameof(FocusShortcut);
}
